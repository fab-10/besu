...
- `engine_newPayload` and `engine_forkchoiceUpdated` now return `INVALID` instead of `SYNCING` for a block whose parent is a known bad block, rather than backward syncing to it. `latestValidHash` is resolved by walking the recorded bad ancestry back to the chain, and is `null` when it cannot be determined. Descendants of a bad block are tracked by header only, so `debug_getBadBlocks` now lists only blocks that failed execution. [#11313](https://github.com/besu-eth/besu/pull/11313)

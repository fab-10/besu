...
- `engine_newPayload` now answers `INVALID` for a payload whose parent is a known bad block, instead of `SYNCING`. The `SYNCING` answer started a backward sync that re-executed the bad block in an endless loop and slowed payload imports to minutes. Backward sync also no longer re-executes known bad blocks and ends the session when a block fails to import.

...
Each method series (`engine_newPayloadV*`, `engine_forkchoiceUpdatedV*`, ...)
...
- `EngineNewPayloadV1 permits EngineNewPayloadV2`, `... V4 permits EngineNewPayloadV5`, and the
  latest version is `final`. The same applies to the other migrated series.
...
- Migrated series take a single `ExecutionEngineJsonRpcMethod.ConstructorArguments` record (built
  ...
  build every version through one shared factory shape (see below).
...
Tests are layered exactly like the production classes: `EngineNewPayloadV5Test extends
EngineNewPayloadV4Test extends ... V1Test`, so **every version class runs all the tests of the

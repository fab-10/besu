...
public class EngineNewPayloadV5Test extends EngineNewPayloadV4Test {
  ...
  protected EngineNewPayloadV1<?> createMethodInstance() {
    return new EngineNewPayloadV5<ExecutionPayloadV4>(
        vertx,
        protocolSchedule,
        protocolContext,
        mergeCoordinator,
  ...
  public void shouldReturnInvalidIfBlockAccessListIsMissing() {
    ...
    final JsonRpcResponse resp = resp(mockV5Payload(header, emptyList(), null, null));
  ...
  public void shouldReturnInvalidIfBlockAccessListHasInvalidHexEncoding() {
    ...
    final JsonRpcResponse resp =
        resp(mockV5Payload(header, emptyList(), null, INVALID_BLOCK_ACCESS_LIST_ENCODING));
  ...
  public void shouldReturnInvalidIfBlockAccessListHasInvalidRlpEncoding() {
    ...
    final JsonRpcResponse resp =
        resp(mockV5Payload(header, emptyList(), null, INVALID_BLOCK_ACCESS_LIST_RLP));
  ...
  protected ExecutionPayloadV4 mockEnginePayload(final BlockHeader header, final List<String> txs) {
    return mockV5Payload(header, txs, null, ENCODED_BLOCK_ACCESS_LIST);
  }
  ...
  protected ExecutionPayloadV4 mockEnginePayload(
    ...
    return mockV5Payload(header, txs, withdrawals, ENCODED_BLOCK_ACCESS_LIST);
  }

  /**
   * Builds a typed {@link ExecutionPayloadV4} with an explicit block-access-list value. Used by the
   * BAL-malformed tests that need to inject specific (or null) BAL strings.
   */
  private ExecutionPayloadV4 mockV5Payload(
      final BlockHeader header,
      final List<String> txs,
      final List<WithdrawalParameter> withdrawals,
